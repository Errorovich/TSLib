...
/// <summary>Per-connection пакетный шифр протокола TeamSpeak: хендшейк TS3INIT1/clientinitiv
/// и шифрование/дешифрование пакетов (AES-EAX + кэш key/nonce).
/// Статические крипто-утилиты (identity, хеши, подписи) — в <see cref="TsCrypt"/>.</summary>
internal sealed class PacketCipher
{
	...
	public PacketCipher(IdentityData identity)
	{
		Reset();
		Identity = identity;
	}
	...
	private byte[] GetSharedSecret(ECPoint publicKeyPoint)
	{
		...
		if (keyArr.Length == 32)
			return TsCrypt.Hash1It(keyArr);
		if (keyArr.Length > 32)
			return TsCrypt.Hash1It(keyArr, keyArr.Length - 32, 32);
		// else keyArr.Length < 32
		var keyArrExt = new byte[32];
		Array.Copy(keyArr, 0, keyArrExt, 32 - keyArr.Length, keyArr.Length);
		return TsCrypt.Hash1It(keyArrExt);
	}
	...
	private E<string> SetSharedSecret(ReadOnlySpan<byte> alpha, ReadOnlySpan<byte> beta, ReadOnlySpan<byte> sharedKey)
	{
		...
		TsCrypt.XorBinary(sharedKey, alpha, alpha.Length, ivStruct);
		TsCrypt.XorBinary(sharedKey.Slice(10), beta, beta.Length, ivStruct.AsSpan(10));

		// creating a dummy signature which will be used on packets which dont use a real encryption signature (like plain voice)
		var buffer2 = TsCrypt.Hash1It(ivStruct, 0, ivStruct.Length);
	...
	internal E<string> CryptoInit2(string license, string omega, string proof, string beta, byte[] privateKey)
	{
		...
		if (!TsCrypt.VerifySign(serverPublicKey.Value, licenseBytes, proofBytes))
	...
	private (byte[] key, byte[] nonce) GetKeyNonce(bool fromServer, ushort packetId, uint generationId, PacketType packetType, bool dummyEncryption)
	{
		...
		if (cacheValue is null || cacheValue.Value.generation != generationId)
		{
			...
			var result = TsCrypt.Hash256It(tmpToHash).AsSpan();
